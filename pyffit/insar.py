...
def prepare_datasets_smooth(insar_files, smooth_files, look_dirs, weights, ref_point, EPSG='32611', rms_min=0.1, mean_low=0.2,rms_min2=0.01,nan_frac_max=0.9, width_min=0.1, width_max=2):
    """
    ...
    for i in range(len(insar_files)):
        ...
        x_samp, y_samp, data_samp, data_samp_std, data_tree, cell_dims, cell_extents, nan_frac = quadtree_unstructured_new(x_smooth.flatten(), y_smooth.flatten(), data_smooth.flatten(), data_index, data_extent,rms_min=rms_min, rms_min2=rms_min2,nan_frac_max=nan_frac_max, mean_low=mean_low,width_min=width_min, width_max=width_max,x_samp=[], y_samp=[], data_samp=[], data_samp_std=[], data_tree=[], data_dims=[], data_extents=[], nan_frac=[])
