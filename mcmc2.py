...
def inversion():
    ...
    n_process = 74                                    # number of threads to use for parallelization
    ...
    reload_axis_option = 1 # this is only used for replotting the triangle plot when reloading the output
    #set it to 1: automatically set the axis range
    #set it to 2: customize the axis range, specify it below:
    custom_limits= [[2.79,2.85],[5.18,5.28],[5.74,5.94],[24.74,24.9],[10.48,11.18],[26.95,27.05],[83,83.5],[3.23,3.41],[-0.25,-0.22]]
    ...
    # Print formatted parameter summary (copy mean values directly into forward.py)
    param_names = list(priors.keys())
    print('\n' + '='*75)
    print('INVERSION RESULTS — Parameter Summary')
    print('='*75)
    print(f'{"Parameter":<15} {"Mean":>12} {"Std":>12} {"Q16":>12} {"Q50":>12} {"Q84":>12}  {"Unit":<5}')
    print('-'*75)
    for i, key in enumerate(param_names):
        print(f'{key:<15} {m_avg[i]:>12.4f} {m_std[i]:>12.4f} {m_q1[i]:>12.4f} {m_q2[i]:>12.4f} {m_q3[i]:>12.4f}  {units[i]:<5}')
    print('='*75)
    print('\n>>> Copy the following line into forward.py as input parameters:')
    print(f'm = [{", ".join([f"{v:.4f}" for v in m_avg])}]')
    print()
    ...
    if inversion_mode == 'reload':
        if reload_axis_option == 1:
            # Option 1: Auto-compute plot limits from sample distribution (default)
            limits = [[float(m_q2[i] - 4*m_std[i]), float(m_q2[i] + 4*m_std[i])] for i in range(len(m_q2))]
        else:    # Option 2: Set custom axis ranges
